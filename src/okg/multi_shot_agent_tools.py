...
class multi_shot_agent:
    def __init__(self, s3_bucket, s3_path, config_path, setting_day, code_in=0):
        ...
        if str(self.config["EXE"]["S3_DEPLOY"]) == "True":
            ...
            self.get_keyword_planner_metrics = self.dynamic_import('google_util/google_ads_api',
                                                                   'get_keyword_planner_metrics')
    ...
    def pre_process_csv(self, df):
        ...
        df = df[(df["Keyword status"] == "Enabled") | (df["Keyword status"] == "ELIGIBLE")]
    ...
    def topsis_score(self, updated_df, weights=[0.1, 0.1, 0.0, 0.5, 0.3],
                        ...
                        entropy_balance=False, threshold_non_data=0.0):
        """
    ...
    def run(self, cb_get_kw_metrics, cb_exec_kw_plan, current_keyword_list, computing=True):
        ...
        if self.code == 0 and self.setting_day.weekday() not in self.running_week_day:
            return [], {}, int(2)
        if str(self.config["EXE"]["S3_DEPLOY"]) == 'True':
            s3_init_client = boto3.client('s3')
            response = s3_init_client.list_objects_v2(
                Bucket=self.s3_bucket,
                Prefix=self.save_dir + self.generated_keyword_data
            )

            if 'Contents' in response:
                print("S3 Gen File already exists.")
                is_init = False
            else:
                print("S3 Gen File does not exists.")
                is_init = True
        elif str(self.config["EXE"]["S3_DEPLOY"]) == 'False':
        ...
        if ((self.setting_day.weekday() in self.running_week_day) and is_init and self.code == 0) or (
           ...
           # )
            ...
            self.RejectedKeywordsReflector.reject_keywords = rejected_kw_list
            tools = [
                ...
                self.RejectedKeywordsReflector(reject_keywords=rejected_kw_list),
                self.SearchVolumeReflector(config=self.config, s3_bucket=self.s3_bucket, save_dir=self.save_dir, dynamic_import_func=self.dynamic_import, df=df,
                                           cb_exec_kw_plan=cb_exec_kw_plan),]
